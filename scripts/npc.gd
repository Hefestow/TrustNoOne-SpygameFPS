...
signal line_started(kind: String)

@export var npc_name: String = "NPC"
@export var dialogue_lines: Array[String] = []
@export var post_dialogue_line: String = ""
@export var beckon_line: String = ""
@export var beckon_delay: float = 2.0
@export var beckon_lifetime: float = 4.5
@export var lock_player: bool = true
@export var play_beckon: bool = false
...
@export var last_line_hold: float = 100
@export var fade_duration: float = 0.25
@export var reopen_cooldown: float = 0.4
...
func _ready() -> void:
	...
	if play_beckon and not beckon_line.is_empty():
...
func _process(delta: float) -> void:
	if not is_typing or current_full_text.is_empty():
		return
	type_timer -= delta
	if type_timer > 0.0:
		return
...
func _handle_interact() -> void:
	...
	if not is_dialogue_open:
		if not can_reopen:
			return
		if has_completed_dialogue and not post_dialogue_line.is_empty():
...
func _start_line(text: String, kind: String) -> void:
	...
	if lock_player and (kind == "main" or kind == "post"):
		_set_player_locked(true)
	_start_voice_chatter()
	line_started.emit(kind)
...
func _set_player_locked(locked: bool) -> void:
	...
	if locked:
		if player.has_method("start_conversation"):
			player.start_conversation(face_anchor)
	else:
